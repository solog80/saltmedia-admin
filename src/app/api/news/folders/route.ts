...
// Creates a folder in the news media library (mesh storage).
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${API_BASE}/createNewsFolder`, {
      method: "POST",
      headers: { Authorization: `Bearer ${SERVICE_ROLE_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({ folder: body.folder }),
      cache: "no-store",
    })
    ...
    console.error("Create news folder error", e)
